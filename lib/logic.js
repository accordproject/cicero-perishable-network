...
function payOut(shipmentReceived) {
    ...
    var server = 'http://localhost:6001';
    // var server = 'http://192.168.1.108:6001';

    // execute the smart clause
    return post( server + '/execute/' + shipment.smartClause, shipmentReceived, {permitResourcesForRelationships: true})
        .then(function (result) {
            // console.log('result: ' + JSON.stringify(result));
            var totalPrice = result.body.response.totalPrice;
 ...
 * @param {org.accordproject.perishablegoods.TemperatureReading} temperatureReading - the TemperatureReading transaction
 * @transaction
 */
